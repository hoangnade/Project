 ...
 * @author manch
 */
@WebServlet(name = "AccountControl", urlPatterns = {"/login"})
public class AccountControl extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (a == null) {
            ...
            if(a.getStatus() == 0){
                request.setAttribute("mess", "Account is active!");
                request.getRequestDispatcher("login.jsp").forward(request, response);
            }
            if(a.getStatus() == 1){
            HttpSession session = request.getSession();
            session.setAttribute("acc", a);
            AccountDetailId accountDetail = new AccountDetailDao().getOneAccountDetail(a.getAccontdetailID());
